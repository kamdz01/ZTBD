...
def run_join_test():
    ...
    try:
        ...
        requests.put(
            f"{couch_url}/customers/_design/customers", json=design_doc_customers
        )
    ...
    filtered_customers = [
        row for row in data.get("rows", []) if row["value"]["count"] >= 1
    ]
    ...
    if customer_ids:
        ...
        for customer_id in customer_ids:
            try:
                customer_view_url = (
                    f"{couch_url}/customers/_design/customers/_view/by_customer_id"
                )
                params = {
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Błąd podczas pobierania klienta {customer_id}, kod: {response.status_code}"
                    )
                    print(response.text)
            except Exception as e:
                ...
                results.append(
                    {
                        ...
                        "last_order_purchase_timestamp": customer_info["value"][
                            "latest_date"
                        ],
                    }
                )
