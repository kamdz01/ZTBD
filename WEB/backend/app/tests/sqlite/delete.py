...
def run_delete_test(limit=None):
    """
    Test złożonego usuwania danych z wielu tabel w SQLite.
    """
    ...
    try:
        ...
        # Dla SQLite, musimy rozdzielić usuwanie i aktualizację (nie obsługuje CTE)
        # Najpierw pobieramy listę product_id z order_items które będą usunięte
        cursor.execute(
            """
        SELECT DISTINCT oi.product_id
        FROM order_items oi
        WHERE oi.order_id IN (SELECT order_id FROM orders_to_delete)
        """
        )
        product_ids = [row[0] for row in cursor.fetchall()]

        # Usuwamy elementy zamówień
        cursor.execute(
            """
