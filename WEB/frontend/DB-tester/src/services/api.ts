...
export const getStructuredResults = async (): Promise<StructuredResults> => {
  ...
  Object.entries(scenariosData.testScenarios).forEach(
    ([scenarioKey, scenarioValue]) => {
      const sizes = scenarioValue.sizes
        ? scenarioValue.sizes.map(String)
        : ["default"];
  ...
  results.forEach((data) => {
    if (data) {
      ...
      grouped[data.scenario][data.database][data.size] = {
        times: data.times,
        rows: data.rows || [],
      };
    }
